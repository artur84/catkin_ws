...
#include <ui_local_planner/latched_stop_rotate_controller.h>
...
#include <ui_local_planner/local_planner_limits.h>
...
bool LatchedStopRotateController::isPositionReached(LocalPlannerUtil* planner_util,
  ...
  if ((latch_xy_goal_tolerance_ && xy_tolerance_latch_) ||
      ui_local_planner::getGoalPositionDistance(global_pose, goal_x, goal_y) <= xy_goal_tolerance) {
...
bool LatchedStopRotateController::isGoalReached(LocalPlannerUtil* planner_util,
  ...
  ui_local_planner::LocalPlannerLimits limits = planner_util->getCurrentLimits();
  ...
  if ((latch_xy_goal_tolerance_ && xy_tolerance_latch_) ||
    ...
    double angle = ui_local_planner::getGoalOrientationAngleDifference(global_pose, goal_th);
    //check to see if the goal orientation has been reached
    if (fabs(angle) <= limits.yaw_goal_tolerance) {
      //make sure that we're actually stopped before returning success
      if (ui_local_planner::stopped(base_odom, rot_stopped_vel, trans_stopped_vel)) {
        return true;
      }
    }
  }
...
bool LatchedStopRotateController::rotateToGoal(
    ...
    Eigen::Vector3f acc_lim,
    double sim_period,
    ui_local_planner::LocalPlannerLimits& limits,
    boost::function<bool (Eigen::Vector3f pos,
                          Eigen::Vector3f vel,
                          Eigen::Vector3f vel_samples)> obstacle_check) {
  double yaw = tf::getYaw(global_pose.getRotation());
...
bool LatchedStopRotateController::computeVelocityCommandsStopRotate(geometry_msgs::Twist& cmd_vel,
  ...
  if (fabs(angle) <= limits.yaw_goal_tolerance) {
    ...
    if ( ! rotating_to_goal_ && !ui_local_planner::stopped(base_odom, limits.rot_stopped_vel, limits.trans_stopped_vel)) {
