...
#include <ui_local_planner/footprint_helper.h>
...
#include <ui_local_planner/map_cell.h>
#include <ui_local_planner/map_grid.h>
...
public:
	 ...
	 * @param try_user_vel_first A bool value to say if we want to send directly the input from the user if no collision is detected.
	 * @param center_dyn_win_on_user_vel A bool value to say if we want to center the dyn_win on user_vel
	 * @param user_vel_cost_scale Weighting factor of the user input when evaluating the trajectories (a bigger value will give more control to the user).
	 * @param face_dir_cost_scale Weighting factor of the user face direction when evaluating the trajectories (a bigger value will give more control to the user).
	 */
	TrajectoryPlanner(WorldModel& world_model,
			...
			bool try_user_vel_first = true,
			bool center_dyn_win_on_user_vel = true,
			double user_vel_cost_scale = 3.0,
			double face_dir_cost_scale = 0.4);
	 ...
	 */
	void reconfigure(UILocalPlannerConfig &cfg);
	...
	/**
	 * @brief  Checks if a trajectory is valid
	...
	/**
	 * @brief  Generate and scores a single trajectory, it gives back the trajectory for visualization
	 * @param x The x position of the robot
	 * @param y The y position of the robot
	 * @param theta The orientation of the robot
	 * @param vx The x velocity of the robot
	 ...
	 * @return The score (as double)
	 */
	double getTrajectory(double x, double y, double theta, double vx, double vy,
			double vtheta, double vx_samp, double vy_samp, double vtheta_samp,
	...
	/** XXX
	 * @brief  Sets the user_vel_ member with the given vx vy and vtheta values, all other values are set to 0.
	 * @param vx vy vz Are the current speeds selected by the user
	 */
	void setUserInput(double vx, double vy, double vtheta);

	/**
	 * @brief  Sets the face_dir_ member with the given roll pitch  and yaw values,
	 * @param roll pitch yaw are the current euler angles of the face of the user
	 */
	void setFaceDir(double roll, double pitch, double yaw);
	/**
	 * @brief  Sets the user_voice_ member with the given roll pitch  and yaw values,
	 * @param roll pitch yaw are the current euler angles of the face of the user
	 */
	void setUserVoice(std_msgs::String &voice);

	/*Puts the planner in escaping mode,
	 * @param escape_vel, the desired escape linear vel. normally -0.1
	 * @param x,y,z the current x,y and z position of the robot.
	 */
	void setEscapeMode(double x, double y, double theta, double escape_vel);
private:
	/**
	...
	ui_local_planner::FootprintHelper footprint_helper_;
	...
	ui_local_planner::WorldModel& world_model_; ///< @brief The world model that the controller uses for collision detection
	...
	bool stuck_left, stuck_right; ///< @brief Booleans to keep the robot from oscillating during rotation, it means the robot persits in going to the same direction
	bool rotating_left, rotating_right; ///< @brief Booleans to keep track of the direction of rotation for the robot

	bool stuck_left_strafe, stuck_right_strafe; ///< @brief (Just for holonomic robots) Booleans to keep the robot from oscillating during strafing
	...
	bool try_user_vel_first_; ///< @brief first try the command given by the user (can lead to collisions)
	bool center_dyn_win_on_user_vel_; ///< @brief center the dynamic window on user input
	...
	//XXX
	double user_vel_cost_; ///< @brief The diference between the velocity asked by the user and the one beeing tested from the dynamic window
	double user_vel_cost_scale_; ///< @brief A weighting factor for the user input vel cost
	double face_dir_cost_scale_; ///< @brief A weighting factor for the user face direction
	geometry_msgs::Twist user_vel_; /// @brief The user intended/selected speed
	geometry_msgs::Twist face_dir_; /// @brief The user face direction
	std_msgs::String user_voice_; /// @brief The user vocal command
	bool new_user_vel_; ///< @brief if the user sent a new user_vel input is true
	bool new_face_dir_; ///< @brief if the user sent a new user_vel input is true
	/**
	...
	/**
	 * @brief  Computes the heading term in dynamic window objective function
	 * G(v,w) = (a*heading(v,w)+b*dist(v,w)+c*velocity(v,w))
	 */
	double headingDiff(int cell_x, int cell_y, double x, double y,
			double heading);
	double userInputDiff(double vx, double vy, double vtheta); //XXX
