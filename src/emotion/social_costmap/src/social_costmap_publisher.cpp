...
namespace social_costmap {
  SocialCostmapPublisher::SocialCostmapPublisher(ros::NodeHandle ros_node, double publish_frequency, std::string global_frame) 
    ...
    visualizer_thread_ = new boost::thread(boost::bind(&SocialCostmapPublisher::mapPublishLoop, this, publish_frequency));
  }

  SocialCostmapPublisher::~SocialCostmapPublisher(){
  ...
  void SocialCostmapPublisher::mapPublishLoop(double frequency){
  ...
  void SocialCostmapPublisher::updateCostmapData(const SocialCostmap& costmap, const std::vector<geometry_msgs::Point>& footprint, const tf::Stamped<tf::Pose>& global_pose){
    ...
    for(unsigned int i = 0; i < costmap.getSizeInCellsX(); i++){
      for(unsigned int j = 0; j < costmap.getSizeInCellsY(); j++){
        ...
        //if(costmap.getCost(i, j) == social_costmap::LETHAL_OBSTACLE || costmap.getCost(i, j) == social_costmap::NO_INFORMATION)
        if(costmap.getCost(i, j) == social_costmap::LETHAL_OBSTACLE)
          raw_obstacles.push_back(p);
        else if(costmap.getCost(i, j) == social_costmap::INSCRIBED_INFLATED_OBSTACLE)
          inflated_obstacles.push_back(p);
        else if(costmap.getCost(i, j) == social_costmap::NO_INFORMATION)
  ...
  void SocialCostmapPublisher::publishFootprint(){
  ...
  void SocialCostmapPublisher::publishCostmap(){
    std::vector< std::pair<double, double> > raw_obstacles, inflated_obstacles, unknown_space;
    double resolution;
