...
V_Cell g_marked;
V_Cell g_unknown;
void voxelCallback(const ros::Publisher& pub_marked, const ros::Publisher& pub_unknown, const social_costmap::VoxelGridConstPtr& grid)
{
  if (grid->data.empty())
  {
...
{
  ros::init(argc, argv, "social_costmap_cloud");
  ros::NodeHandle n;
  ...
  ros::Subscriber sub = n.subscribe<social_costmap::VoxelGrid>("voxel_grid", 1, boost::bind(voxelCallback, pub_marked, pub_unknown, _1));
