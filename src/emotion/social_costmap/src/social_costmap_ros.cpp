...
namespace social_costmap {
  ...
  SocialCostmapROS::SocialCostmapROS(std::string name, tf::TransformListener& tf) : name_(name), tf_(tf), costmap_(NULL),
    ...
    if(publish_voxel_ && map_type == "voxel")
      voxel_pub_ = private_nh.advertise<social_costmap::VoxelGrid>("voxel_grid", 1);
    else
    ...
    if(static_map){
      ...
      map_sub_ = g_nh.subscribe(map_topic, 1, &SocialCostmapROS::incomingMap, this);
    ...
    while(ss >> source){
      ...
      if(data_type == "LaserScan"){
        ...
        filter->registerCallback(boost::bind(&SocialCostmapROS::laserScanCallback, this, _1, observation_buffers_.back()));
        ...
        filter->registerCallback(boost::bind(&SocialCostmapROS::pointCloudCallback, this, _1, observation_buffers_.back()));
        ...
        filter->registerCallback(boost::bind(&SocialCostmapROS::pointCloud2Callback, this, _1, observation_buffers_.back()));
    ...
    if(map_type == "costmap"){
      ...
      boost::recursive_mutex::scoped_lock lock(map_data_lock_);
      costmap_ = new SocialCostmap(map_width, map_height,
      ...
      boost::recursive_mutex::scoped_lock lock(map_data_lock_);
      costmap_ = new VoxelSocialCostmap(map_width, map_height, z_voxels, map_resolution, z_resolution, map_origin_x, map_origin_y, map_origin_z, inscribed_radius,
    ...
    costmap_publisher_ = new SocialCostmapPublisher(private_nh, map_publish_frequency, global_frame_);
    ...
    map_update_thread_ = new boost::thread(boost::bind(&SocialCostmapROS::mapUpdateLoop, this, map_update_frequency));

    costmap_initialized_ = true;

    //Create a time r to check if the robot is moving
    timer_ = private_nh.createTimer(ros::Duration(.1), &SocialCostmapROS::movementCB, this);
    dsrv_ = new dynamic_reconfigure::Server<SocialCostmapConfig>(ros::NodeHandle("~/"+name));
    dynamic_reconfigure::Server<SocialCostmapConfig>::CallbackType cb = boost::bind(&SocialCostmapROS::reconfigureCB, this, _1, _2);
    dsrv_->setCallback(cb);
  }

  void SocialCostmapROS::movementCB(const ros::TimerEvent &event) {
  ...
  void SocialCostmapROS::reconfigureCB(SocialCostmapConfig &config, uint32_t level) {
    ...
    if(!setup_) {
      ...
      //TOD Make sure that this check actually does the right things
      ...
      if(user_params) {
        ...
        if(!config.static_map && !config.rolling_window){
          ...
          map_sub_.shutdown();
          map_initialized_ = false;
          map_sub_ = g_nh.subscribe(config.map_topic, 1, &SocialCostmapROS::incomingMap, this);
        ...
        if(config.map_type == "voxel") {
          ...
          VoxelSocialCostmap *new_map = new VoxelSocialCostmap(size_x, size_y, z_voxels,
          ...
          SocialCostmap *new_map = new SocialCostmap(size_x, size_y, resolution, origin_x, origin_y,
                  inscribed_radius, circumscribed_radius, config.inflation_radius,
      ...
      if(config.map_type == "voxel" && config.publish_voxel_map) {
        ...
        if(voxel_pub_ == NULL) {
          ...
          voxel_pub_ = nh.advertise<social_costmap::VoxelGrid>("voxel_grid", 1);
        }
      }
      else {
        publish_voxel_ = false;
      ...
      if(config.publish_frequency != last_config_.publish_frequency)
      {
        ...
        costmap_publisher_ = new SocialCostmapPublisher(ros::NodeHandle("~/"+name_), map_publish_frequency, global_frame_);
      }
  ...
  double SocialCostmapROS::distanceToLine(double pX, double pY, double x0, double y0, double x1, double y1){
    double A = pX - x0;
    double B = pY - y0;
    double C = x1 - x0;
    double D = y1 - y0;
  ...
  std::vector<geometry_msgs::Point> SocialCostmapROS::loadRobotFootprint(ros::NodeHandle node, double inscribed_radius, double circumscribed_radius){
  ...
  SocialCostmapROS::~SocialCostmapROS(){
    delete dsrv_;
    map_update_thread_shutdown_ = true;
  ...
  void SocialCostmapROS::addObservationBuffer(const boost::shared_ptr<ObservationBuffer>& buffer){
    if(buffer)
      observation_buffers_.push_back(buffer);
  }

  void SocialCostmapROS::laserScanCallback(const sensor_msgs::LaserScanConstPtr& message, const boost::shared_ptr<ObservationBuffer>& buffer){
  ...
  void SocialCostmapROS::pointCloudCallback(const sensor_msgs::PointCloudConstPtr& message, const boost::shared_ptr<ObservationBuffer>& buffer){
  ...
  void SocialCostmapROS::pointCloud2Callback(const sensor_msgs::PointCloud2ConstPtr& message, const boost::shared_ptr<ObservationBuffer>& buffer){
    //buffer the point cloud
    buffer->lock();
  ...
  void SocialCostmapROS::mapUpdateLoop(double frequency){
  ...
  bool SocialCostmapROS::getMarkingObservations(std::vector<Observation>& marking_observations) const {
    bool current = true;
  ...
  bool SocialCostmapROS::getClearingObservations(std::vector<Observation>& clearing_observations) const {
    bool current = true;
  ...
  void SocialCostmapROS::updateMap(){
    tf::Stamped<tf::Pose> global_pose;
    ...
    std::vector<Observation> social_grid_observations; //This is to get the points  generated using the social filter.
    ...
    if(publish_voxel_){
      social_costmap::VoxelGrid voxel_grid;
      ((VoxelSocialCostmap*)costmap_)->getVoxelGridMessage(voxel_grid);
  ...
  void SocialCostmapROS::clearNonLethalWindow(double size_x, double size_y){
    tf::Stamped<tf::Pose> global_pose;
    if(!getRobotPose(global_pose))
      return;
  ...
  void SocialCostmapROS::resetMapOutsideWindow(double size_x, double size_y){
    tf::Stamped<tf::Pose> global_pose;
    if(!getRobotPose(global_pose))
      return;
  ...
  void SocialCostmapROS::getCostmapCopy(SocialCostmap& costmap) const {
    boost::recursive_mutex::scoped_lock lock(lock_);
    costmap = *costmap_;
  }

  void SocialCostmapROS::incomingMap(const nav_msgs::OccupancyGridConstPtr& new_map){
    if(!map_initialized_){
      initFromMap(*new_map);
  ...
  void SocialCostmapROS::initFromMap(const nav_msgs::OccupancyGrid& map){
    boost::recursive_mutex::scoped_lock lock(map_data_lock_);
  ...
  void SocialCostmapROS::updateStaticMap(const nav_msgs::OccupancyGrid& new_map){
  ...
  void SocialCostmapROS::getCostmapWindowCopy(double win_size_x, double win_size_y, SocialCostmap& costmap) const {
    boost::recursive_mutex::scoped_lock lock(lock_);
    tf::Stamped<tf::Pose> global_pose;
    if(!getRobotPose(global_pose)){
  ...
  void SocialCostmapROS::getCostmapWindowCopy(double win_center_x, double win_center_y, double win_size_x, double win_size_y, SocialCostmap& costmap) const {
    boost::recursive_mutex::scoped_lock lock(lock_);
  ...
  unsigned int SocialCostmapROS::getSizeInCellsX() const {
    boost::recursive_mutex::scoped_lock lock(lock_);
    return costmap_->getSizeInCellsX();
  }

  unsigned int SocialCostmapROS::getSizeInCellsY() const {
    boost::recursive_mutex::scoped_lock lock(lock_);
    return costmap_->getSizeInCellsY();
  }

  double SocialCostmapROS::getResolution() const {
    boost::recursive_mutex::scoped_lock lock(lock_);
    return costmap_->getResolution();
  }

  bool SocialCostmapROS::getRobotPose(tf::Stamped<tf::Pose>& global_pose) const {
    ...
    if (current_time.toSec() - global_pose.stamp_.toSec() > transform_tolerance_) {
      ROS_WARN_THROTTLE(1.0, "SocialCostmapROS transform timeout. Current time: %.4f, global_pose stamp: %.4f, tolerance: %.4f",
  ...
  void SocialCostmapROS::clearRobotFootprint(){
    tf::Stamped<tf::Pose> global_pose;
    if(!getRobotPose(global_pose))
      return;
  ...
  std::vector<geometry_msgs::Point> SocialCostmapROS::getRobotFootprint() const {
    return footprint_spec_;
  }

  void SocialCostmapROS::getOrientedFootprint(std::vector<geometry_msgs::Point>& oriented_footprint) const {
    tf::Stamped<tf::Pose> global_pose;
    if(!getRobotPose(global_pose))
      return;
  ...
  void SocialCostmapROS::getOrientedFootprint(double x, double y, double theta, std::vector<geometry_msgs::Point>& oriented_footprint) const {
  ...
  bool SocialCostmapROS::setConvexPolygonCost(const std::vector<geometry_msgs::Point>& polygon, unsigned char cost_value){
    lock_.lock();
  ...
  std::string SocialCostmapROS::getGlobalFrameID() const {
    return global_frame_;
  }

  std::string SocialCostmapROS::getBaseFrameID() const {
    return robot_base_frame_;
  }

  double SocialCostmapROS::getInscribedRadius() const {
    boost::recursive_mutex::scoped_lock lock(lock_);
    return costmap_->getInscribedRadius();
  }

  double SocialCostmapROS::getCircumscribedRadius() const {
    boost::recursive_mutex::scoped_lock lock(lock_);
    return costmap_->getCircumscribedRadius();
  }

  double SocialCostmapROS::getInflationRadius() const {
    boost::recursive_mutex::scoped_lock lock(lock_);
    return costmap_->getInflationRadius();
  }

  void SocialCostmapROS::clearRobotFootprint(const tf::Stamped<tf::Pose>& global_pose){
    std::vector<geometry_msgs::Point> oriented_footprint;
    ...
    if(!costmap_->setConvexPolygonCost(oriented_footprint, social_costmap::FREE_SPACE))
      return;
