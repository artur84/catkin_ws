...
namespace social_costmap{
  VoxelSocialCostmap::VoxelSocialCostmap(unsigned int cells_size_x, unsigned int cells_size_y, unsigned int cells_size_z,
    ...
    : SocialCostmap(cells_size_x, cells_size_y, xy_resolution, origin_x, origin_y, inscribed_radius, circumscribed_radius,
  ...
  VoxelSocialCostmap::VoxelSocialCostmap(social_costmap::SocialCostmap& costmap, 
      double z_resolution, unsigned int cells_size_z, double origin_z, 
      unsigned int mark_threshold, unsigned int unknown_threshold)
    : SocialCostmap(costmap), voxel_grid_(costmap.getSizeInCellsX(), costmap.getSizeInCellsY(), cells_size_z), 
  ...
  VoxelSocialCostmap::~VoxelSocialCostmap(){}

  void VoxelSocialCostmap::initMaps(unsigned int size_x, unsigned int size_y){
    SocialCostmap::initMaps(size_x, size_y);
    voxel_grid_.resize(size_x, size_y, size_z_);
    ROS_ASSERT(voxel_grid_.sizeX() == size_x_ && voxel_grid_.sizeY() == size_y_);
  }

  void VoxelSocialCostmap::resetMaps(){
    SocialCostmap::resetMaps();
    voxel_grid_.reset();
  }

  void VoxelSocialCostmap::resetMapOutsideWindow(double wx, double wy, double w_size_x, double w_size_y){
  ...
  void VoxelSocialCostmap::updateObstacles(const vector<Observation>& observations, priority_queue<CellData>& inflation_queue){
  ...
  void VoxelSocialCostmap::raytraceFreespace(const Observation& clearing_observation){
  ...
  void VoxelSocialCostmap::updateOrigin(double new_origin_x, double new_origin_y){
  ...
  void VoxelSocialCostmap::clearNonLethal(double wx, double wy, double w_size_x, double w_size_y, bool clear_no_info){
  ...
  void VoxelSocialCostmap::getVoxelGridMessage(VoxelGrid& grid){
  ...
  void VoxelSocialCostmap::getPoints(pcl::PointCloud<pcl::PointXYZ>& cloud){
  ...
  void VoxelSocialCostmap::finishConfiguration(social_costmap::SocialCostmapConfig &config) {
