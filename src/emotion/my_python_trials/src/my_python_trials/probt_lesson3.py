...
PS_k_D1 =model.ask(S,D1)
VD1=plValues(D1)
VD1[D1]=4
print PS_k_D1.instantiate(VD1).compile()
...
def my_or(O_,I_):
    O_[D]=int(I_[A]) or int(I_[B])

def my_and_one(O_,I_):
    O_[E]=int(I_[C]) and int(I_[B])

def my_and_two(O_,I_):
    O_[F]=int(I_[D]) and int(I_[E])
...
HC.add_point(0)#regresa true
HC.add_point(1)#regresa true
HC.add_point(1)#regresa true

PC=HC.get_distribution()#Esto en el ejemplo anterior estaba dado por una tabla ahora lo hemos aprendido

HC.add_point(1)
PC=HC.get_distribution()
#A partir de aqui estaba en el ejemplo de la clase pasada
#conditional distribution 
PT_k_C = plDistributionTable(T,C)
