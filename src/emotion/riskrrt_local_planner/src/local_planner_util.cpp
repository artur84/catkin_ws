#include <riskrrt_local_planner/local_planner_util.h>
...
bool LocalPlannerUtil::getGoal(tf::Stamped<tf::Pose>& goal_pose) {
  ...
  return riskrrt_local_planner::getGoalPose(*tf_,
        global_plan_,
        global_frame_,
        goal_pose);
}
...
bool LocalPlannerUtil::getLocalPlan(tf::Stamped<tf::Pose>& global_pose, std::vector<geometry_msgs::PoseStamped>& transformed_plan) {
  ...
  if(!riskrrt_local_planner::transformGlobalPlan(
      *tf_,
      global_plan_,
      global_pose,
      *costmap_,
  ...
  if(limits_.prune_plan) {
    riskrrt_local_planner::prunePlan(global_pose, transformed_plan, global_plan_);
  }
  return true;
}
