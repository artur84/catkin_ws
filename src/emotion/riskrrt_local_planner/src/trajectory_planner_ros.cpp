#include <riskrrt_local_planner/trajectory_planner_ros.h>
#include <sys/time.h>
...
#include <riskrrt_local_planner/goal_functions.h>
#include <geometry_msgs/PolygonStamped.h>
#include <nav_msgs/Path.h>
#include <geometry_msgs/Twist.h>
//Dependencies from package RiskRRT/src
#include "readParameters.hpp"
#include "wheelchair.hpp"
#include "RobotRos.hpp"
#include "LocalMap.hpp"
#include "PlanHandlerRos.hpp"
#include "GoalHandlerRos.hpp"
#include "LocalMapRos.hpp"
#include "HumanHandlerRos.hpp"
#include "IntHandlerRos.hpp"
#include "TreeHandlerRos.hpp"
#include "timeval_math.hpp"
#include "rrt.hpp"
#include "collisionChecker.hpp"

//register this planner as a BaseLocalPlanner plugin
PLUGINLIB_DECLARE_CLASS(base_local_planner, TrajectoryPlannerROS,
		riskrrt_local_planner::TrajectoryPlannerROS, nav_core::BaseLocalPlanner)

namespace riskrrt_local_planner
{

void TrajectoryPlannerROS::reconfigureCB(RISKRRTLocalPlannerConfig &config,
		uint32_t level)
{
...
/*
 * This creator will just set the initializad_ flag to false, Just in case someone call it in this bad way.
 */
		...
		tf::TransformListener* tf, costmap_2d::Costmap2DROS* costmap_ros)
{
	if (!isInitialized())
	{
		...
		double ui_cost_scale; //weighting factor
		private_nh.param("ui_scoring", ui_scoring, true);
		private_nh.param("ui_cost_scale", ui_cost_scale, 3.0); //
		...
		tc_ = new TrajectoryPlanner(*world_model_, costmap_,
				...
				angular_sim_granularity, ui_scoring, ui_cost_scale);
		...
		dsrv_ = new dynamic_reconfigure::Server<RISKRRTLocalPlannerConfig>(
				private_nh);
		dynamic_reconfigure::Server<RISKRRTLocalPlannerConfig>::CallbackType cb =
				boost::bind(&TrajectoryPlannerROS::reconfigureCB, this, _1, _2);
		dsrv_->setCallback(cb);
		user_vel_handler_.initReader();
		/*
		 * Riskrrt parameters; could be loaded from a yaml file
		 */
		private_nh.param("timeStep", riskrrt_params_.timeStep,0.5);
		private_nh.param("maxDepth", riskrrt_params_.maxDepth,25);
		private_nh.param("nv", riskrrt_params_.nv,12); //equivalent to vx_samples in ROS
		private_nh.param("nphi", riskrrt_params_.nphi,20); //equivalent to vtheta_samples in ROS
		private_nh.param("pThreshold", riskrrt_params_.pThreshold,0.7);
		private_nh.param("psFlag", riskrrt_params_.psFlag,true);
		private_nh.param("intFlag", riskrrt_params_.intFlag,true);
		private_nh.param("GP", riskrrt_params_.GP,0);
		private_nh.param("dataDirGP", riskrrt_params_.dataDirGP,string(""));
		private_nh.param("loopRate", riskrrt_params_.loopRate, 2);
		private_nh.param("pGoal", riskrrt_params_.pGoal, .01);
		/*
		 * TODO: Init all Riskrrt variables
		 */
		ifstream parFile2(riskrrt_robot_config_dir_.c_str() , ifstream::in );
		assert (parFile2.good());
		riskrrt_robot_(riskrrt_robot_config_dir_, riskrrt_params_.timeStep);
		ros_robot_.init(&riskrrt_robot_);
		ros_robot_.initLoc();
		riskrrt_(&riskrrt_robot_,&riskrrt_params_);
		hum_detected_.initSubs();
		int_detected_.initSubs();
		ros_tree_.init(riskrrt_.MAX_DEPTH);
		ped_predict_vect_.clear();
		//If we want to use Gaussian Process then read the files
		if(riskrrt_params_.GP){
			if(riskrrt_params_.dataDirGP.size()>1)
				gp_load_ = new Loader(riskrrt_params_.dataDirGP.c_str());
			else
				ROS_INFO("Data file for GP trajectories not specified");
		}
		timeStepv_.tv_sec = (int) floor(riskrrt_._timeStep);
		timeStepv_.tv_usec = double (riskrrt_._timeStep - timeStepv_.tv_sec)  * 1000000;
		//Filling the time vector for predictions
		ped_predict_time_vect_.clear();
		for ( int i=-1; i< riskrrt_.MAX_DEPTH; i++){
			ped_predict_time_vect_.push_back((i+1)* riskrrt_._timeStep);
		}
		ped_predict_marker_.init();
		...
		geometry_msgs::Twist& cmd_vel)
{
	...
	tf::Stamped<tf::Pose> drive_cmds; //A vector of poses (is it the part of the global plan that is being followed??)
					...
					<= xy_goal_tolerance_))
	{
		...
		else
		{
			...
			timeLoc_.tv_sec = t.sec;
			timeLoc_.tv_usec = t.nsec/1000;
			fill_riskrrt_map();
			//we need to call the next two lines to make sure that the trajectory
			//planner updates its path distance and goal distance grids
			tc_->updatePlan(transformed_plan);
			//Generate and display all possible trajectories to be evaluated
			...
			if (!rotating_to_goal_
					&& !riskrrt_local_planner::stopped(base_odom,
	...
	Trajectory path = tc_->findBestPathVector(global_pose, robot_vel,
			drive_cmds, evaluated_trajs_); //This is the best local path for the given robot speed, pose and desired velocity
	map_viz_.publishCostCloud(costmap_);
	...
	ROS_DEBUG_NAMED("trajectory_planner_ros",
			"A valid velocity command of (%.2f, %.2f, %.2f) was found for this cycle.", cmd_vel.linear.x, cmd_vel.linear.y, cmd_vel.angular.z);
	...
	Trajectory partial_path = evaluated_trajs_[0];
	//To print all the evaluated paths (for visualization purposes) however it can be removed to improve performance
	for (unsigned int j = 0; j < evaluated_trajs_.size() / 2 - 2; ++j)
	{
		partial_path = evaluated_trajs_[j * 2];
		//Drawing forward
		for (unsigned int i = 0; i < partial_path.getPointsSize(); ++i)
		{
			double p_x, p_y, p_th;
			partial_path.getPoint(i, p_x, p_y, p_th);
			tf::Stamped<tf::Pose> p = tf::Stamped<tf::Pose>(
					tf::Pose(tf::createQuaternionFromYaw(p_th),
							tf::Point(p_x, p_y, 0.0)), ros::Time::now(),
					global_frame_);
			geometry_msgs::PoseStamped pose;
			tf::poseStampedTFToMsg(p, pose);
			partial_plan.push_back(pose);
		}
		//Drawing backward on the same arch (to avoid straight lines between archs)
		for (int i = partial_path.getPointsSize() - 1; i >= 0; --i)
		{
			double p_x, p_y, p_th;
			partial_path.getPoint(i, p_x, p_y, p_th);
			tf::Stamped<tf::Pose> p = tf::Stamped<tf::Pose>(
...
bool TrajectoryPlannerROS::checkTrajectory(double vx_samp, double vy_samp,
		double vtheta_samp, bool update_map)
{
	...
	if (costmap_ros_->getRobotPose(global_pose))
	{
		...
		return tc_->checkTrajectory(global_pose.getOrigin().x(),
...
bool TrajectoryPlannerROS::isGoalReached()
{
	...
	return riskrrt_local_planner::isGoalReached(*tf_, global_plan_, costmap_,
			global_frame_, global_pose, base_odom, rot_stopped_velocity_,
			trans_stopped_velocity_, xy_goal_tolerance_, yaw_goal_tolerance_);
}

void TrajectoryPlannerROS::set_riskrrt_map()
{

	riskrrt_occ_grid_.map.cell_size = costmap_ros_->getResolution();
	riskrrt_occ_grid_.map.width = costmap_.getSizeInCellsX();
	riskrrt_occ_grid_.map.height = costmap_.getSizeInCellsY();//TODO: check the orientation
	riskrrt_occ_grid_.map.pos.x = costmap_.getOriginX();
	riskrrt_occ_grid_.map.pos.y = costmap_.getOriginY();
	riskrrt_occ_grid_.map.pos.theta = 0.0;

	riskrrt_occ_grid_.map.data.resize(riskrrt_occ_grid_.map.width* riskrrt_occ_grid_.map.height);
	int i = 0;
	for(unsigned int my=0; my<riskrrt_occ_grid_.map.height ; my++)
	{
		for(unsigned int mx=0; mx<riskrrt_occ_grid_.map.width ; mx++)
		{
			if(costmap_.getCost(mx,my)==-1)
			{
				riskrrt_occ_grid_.map.data[i] = 0.5;
			}else{
				riskrrt_occ_grid_.map.data[i] = costmap_.getCost(mx,my)/100.;
			}
			i++;
		}
	}
