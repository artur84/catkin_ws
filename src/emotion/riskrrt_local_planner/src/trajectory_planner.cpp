#include <riskrrt_local_planner/trajectory_planner.h>
...
void TrajectoryPlanner::reconfigure(RISKRRTLocalPlannerConfig &cfg)
{
	RISKRRTLocalPlannerConfig config(cfg);
	...
	ui_cost_scale_ = config.ui_cost_scale;
...
TrajectoryPlanner::TrajectoryPlanner(WorldModel& world_model,
		...
		double sim_period, double angular_sim_granularity, bool ui_scoring, double ui_cost_scale,
		bool p_space_scoring, bool o_space_scoring,bool GP_predict, double pThreshold,
		std::string dataDirGP, int loopRate, double pGoal):
		path_map_(costmap.getSizeInCellsX(), costmap.getSizeInCellsY()), goal_map_(
				...
				sim_period), ui_scoring_(ui_scoring), ui_cost_scale_(ui_cost_scale), p_space_scoring_(p_space_scoring) ,
				o_space_scoring_(o_space_scoring),GP_predict_(GP_predict), pThreshold_(pThreshold),
				dataDirGP_(dataDirGP), loopRate_(loopRate), pGoal_(pGoal)
{
		...
		double impossible_cost, Trajectory& traj)
{
	...
	if (ui_scoring_)
	{
		ui_diff_cost_ = userVelDiff(vx_samp, vy_samp, vtheta_samp);
		if (!heading_scoring_)
		{
			cost = pdist_scale_ * path_dist + goal_dist * gdist_scale_
					+ occdist_scale_ * occ_cost
					+ ui_cost_scale_ * ui_diff_cost_;
		}
		else
		{
	...
	ROS_WARN(
			"cost: %f, vtheta_traj: %f, vtheta_user: %f", cost, vtheta_samp, ui_vel_.angular.z);
}
 ...
 */
Trajectory TrajectoryPlanner::createTrajectories(double x, double y,
		...
		tf::Stamped<tf::Pose>& drive_velocities)
{
	...
	/*
	 ...
	 char buf[4096];
	 sprintf(buf, "riskrrt_local_planner.ppm");
	 FILE *fp = fopen(buf, "w");
	 if(fp){
	 fprintf(fp, "P3\n");
		...
		)
{
	...
	if  (ui_vel_.linear.x < 0)//if the user gives a "backward" command then the min linear vel should be negative whit the value given by the user
	{
		ROS_WARN("if");
		min_vel_x =  -min_vel_x;
		max_vel_x =  -max_vel_x;
	...
	min_vel_theta = ui_vel_.angular.z - fabs(min_vel_th_);
	max_vel_theta = ui_vel_.angular.z + fabs(max_vel_th_);
...
double TrajectoryPlanner::userVelDiff(double vx, double vy, double vtheta)
{
	double user_vel_diff = (ui_vel_.angular.z - vtheta)
			* (ui_vel_.angular.z - vtheta); //TODO: check which function fits better to user needs.
	//Here I think I could use the predicted path to have a function cost taking into account how the user will change the move of his head over the time;
	// 							X <--Destination
	//            .      .      .         .
	//         .       .        .           .
	//      *---------*---------*------------*
...
void TrajectoryPlanner::setUiVel(double vx, double vy, double vtheta)
{
	ui_vel_.linear.x = vx;
	ui_vel_.linear.y = vy;
	ui_vel_.linear.z = 0;
